namespace UPlan.Frame.DockForm
{
    ...
    internal class VS2005AutoHideStrip : AutoHideStripBase
    {
        ...
        private Rectangle GetLogicalTabStripRectangle(DockState dockState, bool transformed)
        {
            ...
            return this.GetTabStripRectangleInfo(dockState, transformed);
        }

        private bool GetRectangleAttrib(DockState dockState, ref int x, ref int y, ref int width, ref int height)
        {
            return (this.RectangleDockLeft(dockState, ref x, ref y, ref width, ref height) || (this.RectangleDockRight(dockState, ref x, ref y, ref width, ref height) || (this.RectangleDockTop(dockState, ref x, ref y, ref width, ref height) || this.RectangleDockBottom(dockState, ref x, ref y, ref width, ref height))));
        }
        ...
        private Rectangle GetTabStripRectangleInfo(DockState dockState, bool transformed)
        {
            int num2;
            int num3;
            int x = num2 = num3 = 0;
            int height = this.MeasureHeight();
            if (!this.GetRectangleAttrib(dockState, ref x, ref num2, ref num3, ref height))
            {
                return Rectangle.Empty;
            }
            if (!transformed)
            {
                return new Rectangle(x, num2, num3, height);
            }
            return this.GetTransformedRectangle(dockState, new Rectangle(x, num2, num3, height));
        }
        ...
        private bool RectangleDockBottom(DockState dockState, ref int x, ref int y, ref int width, ref int height)
        {
            ...
            if ((dockState == DockState.DockBottomAutoHide) && (num4 > 0))
            {
                ...
                width = (base.Width - ((count == 0) ? 0 : height)) - ((num2 == 0) ? 0 : height);
                return true;
            }
            return false;
        }

        private bool RectangleDockLeft(DockState dockState, ref int x, ref int y, ref int width, ref int height)
        {
            int count = base.GetPanes(DockState.DockLeftAutoHide).Count;
            int num2 = base.GetPanes(DockState.DockTopAutoHide).Count;
            int num3 = base.GetPanes(DockState.DockBottomAutoHide).Count;
            if ((dockState == DockState.DockLeftAutoHide) && (count > 0))
            {
                x = 0;
                y = (num2 == 0) ? 0 : height;
                width = (base.Height - ((num2 == 0) ? 0 : height)) - ((num3 == 0) ? 0 : height);
                return true;
            }
            return false;
        }

        private bool RectangleDockRight(DockState dockState, ref int x, ref int y, ref int width, ref int height)
        {
            int count = base.GetPanes(DockState.DockLeftAutoHide).Count;
            int num2 = base.GetPanes(DockState.DockRightAutoHide).Count;
            int num3 = base.GetPanes(DockState.DockTopAutoHide).Count;
            ...
            if ((dockState == DockState.DockRightAutoHide) && (num2 > 0))
            {
                ...
                y = (num3 == 0) ? 0 : height;
                width = (base.Height - ((num3 == 0) ? 0 : height)) - ((num4 == 0) ? 0 : height);
                return true;
            }
            return false;
        }

        private bool RectangleDockTop(DockState dockState, ref int x, ref int y, ref int width, ref int height)
        {
            int count = base.GetPanes(DockState.DockLeftAutoHide).Count;
            int num2 = base.GetPanes(DockState.DockRightAutoHide).Count;
            int num3 = base.GetPanes(DockState.DockTopAutoHide).Count;
