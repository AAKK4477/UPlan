...
namespace UPlan.Frame.DockForm
{
    ...
    public sealed class NestedPaneCollection : ReadOnlyCollection<DockPane>
    {
        ...
        {
            if ((base.Count == 0) && (this.Container.DockState == UPlan.Frame.DockForm.DockState.Float))
            {
                FloatWindow container = (FloatWindow) this.Container;
                if (!(container.Disposing || container.IsDisposed))
                {
                    UPlan.Frame.DockForm.NativeMethods.PostMessage(((FloatWindow) this.Container).Handle, 0x401, 0, 0);
                }
            }
        }
