...
namespace UPlan.Frame.DockForm
{
	...
	{
		private Dictionary<DockStyle, DockAlignment> m_StyleAlignmentMap = new Dictionary<DockStyle, DockAlignment>();
		...
		private NotifyWindowStateChangeDelegate m_NotifyWindowStateChanged = null;
		...
		private DockState DefaultDockState
		{
			get
			{
				...
				else if ((this.DockAreas & DockAreas.Document) != (DockAreas)0)
				{
					result = DockState.Document;
				}
				else if ((this.DockAreas & DockAreas.DockRight) != (DockAreas)0)
				{
					result = DockState.DockRight;
				}
				else if ((this.DockAreas & DockAreas.DockLeft) != (DockAreas)0)
				{
					result = DockState.DockLeft;
				}
				else if ((this.DockAreas & DockAreas.DockBottom) != (DockAreas)0)
				{
					result = DockState.DockBottom;
				}
				else if ((this.DockAreas & DockAreas.DockTop) != (DockAreas)0)
				{
		...
		private DockState DefaultShowState
		{
			get
			{
				...
				else if ((this.DockAreas & DockAreas.Float) != (DockAreas)0)
				{
			...
			set
			{
				if (this.m_dockState != value)
				{
					...
					if (this.IsHidden)
					{
						...
						{
							this.SetDockState(true, value, this.Pane);
						}
					}
					else if (value == DockState.Hidden)
					{
						this.IsHidden = true;
					}
					else
		...
		public NotifyWindowStateChangeDelegate NotifyWindowStateChanged
		{
			get
			{
				return this.m_NotifyWindowStateChanged;
			}
			set
			{
				this.m_NotifyWindowStateChanged = value;
			}
		}
		...
		private void GetPane(DockState visibleState)
		{
		...
		private void SetDockState(bool isHidden, DockState visibleState, DockState oldDockState)
		{
		...
		private bool OnHiding()
		{
			EventHandler<CancelEventArgs> hiding = this.Hiding;
			CancelEventArgs cancelEventArgs = new CancelEventArgs(false);
			if (hiding != null)
			{
				hiding(this.Form, cancelEventArgs);
			}
		...
		public void Hide()
		{
			...
			{
				this.IsHidden = true;
				if (null != this.m_NotifyWindowStateChanged)
				{
					this.m_NotifyWindowStateChanged(this.Content);
				}
			}
		}
		...
		{
			this.m_dockPanel.RemovingContent(this.Content, e);
		}
		...
		private void InitDockStyleMap()
		{
			this.m_StyleAlignmentMap.Add(DockStyle.Left, DockAlignment.Left);
			this.m_StyleAlignmentMap.Add(DockStyle.Right, DockAlignment.Right);
			this.m_StyleAlignmentMap.Add(DockStyle.Top, DockAlignment.Top);
			this.m_StyleAlignmentMap.Add(DockStyle.Bottom, DockAlignment.Bottom);
		}
